...
export default function CreatineCard({ doseG, taken }: Props) {
  ...
  const [optimisticTaken, setOptimisticTaken] = useOptimistic(taken, (_prev, next: boolean) => next)

  function toggle() {
    const next = !optimisticTaken
    startTransition(async () => {
      setOptimisticTaken(next)
      await toggleCreatine(next)
    })
  }
  ...
  return (
    <div className="bg-surface-container border border-surface-container-highest rounded-xl p-md">
      <div className="flex items-center gap-2 mb-md">
        ...
        <span className="text-[10px] text-secondary bg-surface-container-high px-2 py-0.5 rounded-full ml-auto shrink-0">resets daily</span>
      </div>

      <div className={`flex items-center gap-2 bg-surface-container-high p-sm rounded-lg transition-opacity ${optimisticTaken ? 'opacity-60' : ''}`}>
        <span className="material-symbols-outlined text-secondary text-[20px] shrink-0">science</span>
        <div className="flex-1 min-w-0">
          <p className={`text-body-sm text-on-surface font-semibold ${optimisticTaken ? 'line-through' : ''}`}>Creatine</p>
          <p className="text-[10px] text-secondary">{doseG}g daily</p>
        </div>
        <button
          onClick={toggle}
          disabled={isPending}
          title={optimisticTaken ? 'Mark as not taken' : 'Mark as taken'}
        ...
        >
          <span
            ...
            style={optimisticTaken ? { fontVariationSettings: "'FILL' 1" } : undefined}
          >
            check_circle
          </span>
        </button>
